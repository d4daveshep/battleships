...
class Ship:
    ...
    def hit(self, coordinate: Coordinate) -> bool:
        """Return True if hit is valid (ship is at that position)"""
        if coordinate in self.positions:
    ...
    def place_ship(self, start: Coordinate, direction: Direction) -> List[Coordinate]:
        ...
        for i in range(self.ship_type.length):
            ...
            if not (0 <= pos.row <= 9 and 0 <= pos.col <= 9):
                raise ValueError(f"Ship placement would go out of bounds at {pos.to_string()}")
