...
class Player:
    def __init__(self, name: str, is_computer: bool = False):
        ...
        self.hits_made: Dict[ShipType, List[int]] = {}  # ship_type -> list of round numbers
    ...
    def fire_shots(self, targets: List[Coordinate], round_number: int) -> List[Coordinate]:
        """Fire shots at the specified coordinates"""
        if len(targets) > self.get_available_shots():
            raise ValueError(f"Cannot fire {len(targets)} shots, only {self.get_available_shots()} available")
    ...
    def record_hits_made(self, ship_hits: Dict[ShipType, int], round_number: int) -> None:
    ...
    def get_shots_fired_in_round(self, round_number: int) -> List[Coordinate]:
        ...
        return [coord for coord, round_num in self.board.shots_fired.items() 
    ...
    def get_shots_received_in_round(self, round_number: int) -> List[Coordinate]:
        ...
        return [coord for coord, round_num in self.board.shots_received.items() 
                if round_num == round_number]
