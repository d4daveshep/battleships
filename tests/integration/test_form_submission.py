...
class TestLoginFormSubmission:
    ...
    def test_valid_login_computer_mode_redirects_to_game(
        self, client, valid_computer_mode_data
    ):
        # Test that valid login with computer mode redirects to game
        response = client.post(
            "/", data=valid_computer_mode_data, follow_redirects=False
        )
...
class TestPlayerNameValidationEndpoint:
    ...
    def test_validation_endpoint_preserves_quotes(self, client):
        ...
        assert (
            "Player name can only contain letter, numbers and spaces" in response.text
        )
