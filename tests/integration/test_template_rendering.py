...
class TestTemplateRendering:
    ...
    def test_error_state_template_rendering(self, client):
        ...
        response = client.post(
            "/", data={"player_name": "X", "game_mode": "human"}
        )  # Too short
    ...
    def test_success_state_preserves_valid_input(self, client):
        ...
        response = client.post(
            "/",
            data={"player_name": "ValidPlayer", "game_mode": "human"},
...
class TestLeaveLobbyTemplateRendering:
    ...
    def test_leave_lobby_button_has_correct_attributes(self, client):
        ...
        assert (
            "action=" in response.text
            or "onclick=" in response.text
            or "hx-" in response.text
        )
    ...
    def test_lobby_template_context_includes_player_name(self, client):
        ...
        assert (
            'name="player_name"' in response.text
            or 'value="ContextTest"' in response.text
        )
...
class TestLobbyPlayersPollingWithGameRequests:
    ...
    def test_lobby_players_endpoint_includes_game_request_notification(
    ...
    def test_lobby_players_endpoint_includes_accept_decline_buttons(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_shows_pending_response_status(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_disables_select_buttons_during_pending_response(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_no_game_request_notification_when_none_pending(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_shows_game_confirmation_message_after_accept(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_shows_decline_confirmation_message(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_real_time_status_updates_after_request_sent(
        self, client: TestClient
    ):
    ...
    def test_lobby_players_endpoint_template_structure_supports_all_game_request_elements(
        self, client: TestClient
    ):
