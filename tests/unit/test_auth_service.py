...
class TestAuthService:
    ...
    def test_valid_player_name(self):
        ...
        result = self.auth_service.validate_player_name("John", strip_quotes=False)
        
        assert result.is_valid is True
        assert result.error_message == ""
        assert result.css_class == "valid"
    
    def test_valid_player_name_with_spaces(self):
        # Test valid player name with spaces
        result = self.auth_service.validate_player_name("John Doe", strip_quotes=False)
        
        assert result.is_valid is True
        assert result.error_message == ""
        assert result.css_class == "valid"
    
    def test_valid_player_name_with_numbers(self):
        # Test valid player name with numbers
        result = self.auth_service.validate_player_name("Player123", strip_quotes=False)
    ...
    def test_empty_player_name(self):
        ...
        result = self.auth_service.validate_player_name("", strip_quotes=False)
    ...
    def test_player_name_too_short(self):
        ...
        result = self.auth_service.validate_player_name("A", strip_quotes=False)
    ...
    def test_player_name_too_long(self):
        ...
        long_name = "A" * 21
        result = self.auth_service.validate_player_name(long_name, strip_quotes=False)
        
        assert result.is_valid is False
        assert result.error_message == "Player name must be between 2 and 20 characters"
        assert result.css_class == "error"
    
    def test_player_name_at_min_length(self):
        # Test player name at minimum length (2 chars)
        result = self.auth_service.validate_player_name("AB", strip_quotes=False)
        
        assert result.is_valid is True
        assert result.error_message == ""
        assert result.css_class == "valid"
    
    def test_player_name_at_max_length(self):
        # Test player name at maximum length (20 chars)
        max_name = "A" * 20
        result = self.auth_service.validate_player_name(max_name, strip_quotes=False)
    ...
    def test_player_name_with_special_characters(self):
        ...
        result = self.auth_service.validate_player_name("Player@123", strip_quotes=False)
        
        assert result.is_valid is False
        assert result.error_message == "Player name can only contain letter, numbers and spaces"
        assert result.css_class == "error"
    
    def test_player_name_with_quotes_no_strip(self):
        # Test player name with quotes when strip_quotes=False
        result = self.auth_service.validate_player_name('"Player"', strip_quotes=False)
        
        assert result.is_valid is False
        assert result.error_message == "Player name can only contain letter, numbers and spaces"
        assert result.css_class == "error"
    
    def test_player_name_with_quotes_strip_enabled(self):
        # Test player name with quotes when strip_quotes=True
        result = self.auth_service.validate_player_name('"Player"', strip_quotes=True)
        
        assert result.is_valid is True
        assert result.error_message == ""
        assert result.css_class == "valid"
    
    def test_player_name_with_single_quotes_strip_enabled(self):
    ...
    def test_player_name_with_mixed_quotes_strip_enabled(self):
    ...
    def test_quoted_empty_name_strip_enabled(self):
        # Test empty quoted name when strip_quotes=True
        result = self.auth_service.validate_player_name('""', strip_quotes=True)
        
        assert result.is_valid is False
        assert result.error_message == "Player name is required"
        assert result.css_class == "error"
    
    def test_whitespace_with_strip_quotes(self):
