...
app.add_middleware(
    SessionMiddleware, secret_key="fox-the-navy-secret-key-change-in-production"
)
...
class GameManager:
    ...
    @staticmethod
    def create_game(
        request: Request, player1_name: str, player2_name: str, vs_computer: bool = True
    ) -> GameState:
...
async def new_game(
    ...
    return JSONResponse(
        {"status": "success", "game_id": GameManager.get_game_id(request)}
    )
...
async def get_board(
    request: Request, player_name: str, game: GameState = Depends(get_game_or_404)
):
...
async def get_shots_fired_board(
    request: Request, player_name: str, game: GameState = Depends(get_game_or_404)
):
    ...
    return templates.TemplateResponse(
        "components/shots_board.html", {"request": request, "player": player}
    )


@app.get("/game/hits-made/{player_name}")
async def get_hits_made(
    request: Request, player_name: str, game: GameState = Depends(get_game_or_404)
):
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "player": player, "ship_types": list(ShipType)},
    )
    ...
    try:
        ...
        if success:
            ...
            if len(game.player1.board.ships) == len(ShipType):
                # Start game if ready
                if not game.player2.is_computer or len(game.player2.board.ships) == len(
                    ShipType
                ):
                    game.start_game()
                else:
                    ...
                    if not success:
                        return JSONResponse(
                            {
                                "status": "error",
                                "message": "Failed to auto-place computer ships",
                            }
                        )
...
async def auto_place_ships(
    request: Request, game: GameState = Depends(get_game_or_404)
):
    ...
    if success:
        ...
        return JSONResponse(
            {"status": "error", "message": "Failed to auto-place ships"}
        )


@app.post("/game/submit-shots")
async def submit_shots(
    request: Request, shots: str = Form(...), game: GameState = Depends(get_game_or_404)
):
    ...
    try:
        ...
        if success:
            ...
            if game.player2.is_computer and not game.is_player_turn_complete(
                game.player2.name
            ):
                # Simple computer AI
            ...
            return JSONResponse(
                {"status": "error", "message": "Failed to submit shots"}
            )
...
async def get_round_results(
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "round_result": round_result, "game": game},
    )
...
async def shot_controls(request: Request, game: GameState = Depends(get_game_or_404)):
    ...
    return templates.TemplateResponse(
        "components/shot_controls.html", {"request": request, "game": game}
    )


@app.get("/game/ship-placement")
async def ship_placement_form(
    request: Request, game: GameState = Depends(get_game_or_404)
):
    ...
    remaining_ships = [
        ship_type for ship_type in ShipType if ship_type not in placed_ships
    ]
